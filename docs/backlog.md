...
|---|---|---|---|
| T-001 | Criar schema de banco `clube_{id}` via Prisma migration ao onboarding | 1d | S1 |
...
| T-021 | Integração Asaas: `POST /v3/payments` com dados do sócio + valor do plano | 1d | S2 |
| T-022 | Salvar `pix_cob_id` e `qr_code_image` na tabela `charges` | 0.5d | S2 |
| T-023 | Job BullMQ: disparar geração de cobranças todo dia 1 às 08h (cron) | 0.5d | S2 |
| T-024 | Tratamento de falha: retry 3x com backoff; setar status `PENDING_RETRY` após exaustão | 1d | S2 |
...
| ID | Task Técnica | Esforço | Sprint |
|---|---|---|---|
| T-026 | Endpoint `POST /webhooks/asaas` com validação HMAC-SHA256 do header `X-Asaas-Signature` | 1d | S2 |
| T-027 | Handler para evento `PAYMENT_RECEIVED`: cria `payments`, atualiza `charge` e `member` | 1d | S2 |
| T-028 | Idempotência: checar se `gateway_txid` já existe antes de processar | 0.5d | S2 |
...
| T-047 | Pipeline CI: GitHub Actions com lint + test + build em todo PR | 0.5d | S1 |
| T-048 | Testes E2E com Playwright: fluxo de login, cadastro de sócio, geração de cobrança | 2d | S3 |

---

## Todas as Tasks — Visão Flat

| ID | Épico | Descrição resumida | Esforço | Sprint |
|---|---|---|---|---|
| T-001 | Onboarding | Criar schema `clube_{id}` no onboarding | 1d | S1 |
| T-002 | Onboarding | Endpoint `POST /api/clubs` | 0.5d | S1 |
| T-003 | Onboarding | Tela multi-step de onboarding | 1d | S1 |
| T-004 | Onboarding | Upload e resize de logo | 0.5d | S1 |
| T-005 | Onboarding | E-mail de boas-vindas | 0.5d | S1 |
| T-006 | Auth | JWT + refresh token httpOnly cookie | 1d | S1 |
| T-007 | Auth | Endpoints login / refresh / logout | 0.5d | S1 |
| T-008 | Auth | Tela de login responsiva | 0.5d | S1 |
| T-009 | Auth | Middleware de autenticação Fastify | 0.5d | S1 |
| T-010 | Auth | RBAC Admin / Treasurer com guards | 1d | S1 |
| T-011 | Sócios | Endpoint `POST /api/members` | 0.5d | S1 |
| T-012 | Sócios | Parser CSV com validação por linha | 1d | S1 |
| T-013 | Sócios | Bulk insert com upsert por CPF | 0.5d | S1 |
| T-014 | Sócios | Tela de listagem com busca e filtros | 1d | S1 |
| T-015 | Sócios | Tela de cadastro/edição individual | 0.5d | S1 |
| T-016 | Sócios | Template CSV de exemplo para download | 0.25d | S1 |
| T-017 | Planos | CRUD de planos via API | 0.5d | S1 |
| T-018 | Planos | Tela de gerenciamento de planos | 0.5d | S1 |
| T-019 | Planos | Validação de plano ativo antes de cobrar | 0.25d | S1 |
| T-020 | Cobranças | `ChargeService.generateMonthly()` | 1d | S2 |
| T-021 | Cobranças | Integração Asaas `POST /v3/payments` | 1d | S2 |
| T-022 | Cobranças | Salvar `pix_cob_id` e QR Code | 0.5d | S2 |
| T-023 | Cobranças | Job cron dia 1 às 08h (BullMQ) | 0.5d | S2 |
| T-024 | Cobranças | Retry 3x com backoff exponencial | 1d | S2 |
| T-025 | Cobranças | Endpoint manual de geração de cobrança | 0.5d | S2 |
| T-026 | Webhook | Endpoint webhook com HMAC-SHA256 | 1d | S2 |
| T-027 | Webhook | Handler `PAYMENT_RECEIVED` | 1d | S2 |
| T-028 | Webhook | Idempotência por `gateway_txid` | 0.5d | S2 |
| T-029 | Webhook | Resposta 200 imediata + job assíncrono | 0.5d | S2 |
| T-030 | Webhook | Teste de integração de webhook | 0.5d | S2 |
| T-031 | WhatsApp | `WhatsAppService` com abstração de provider | 1d | S2 |
| T-032 | WhatsApp | Templates D-3, D-0, D+3 configuráveis | 0.5d | S2 |
| T-033 | WhatsApp | Job D-3 (lembrete pré-vencimento) | 0.5d | S2 |
| T-034 | WhatsApp | Job D+3 (cobrança inadimplente) | 0.5d | S2 |
| T-035 | WhatsApp | Rate limiter Redis sliding window | 0.5d | S2 |
| T-036 | WhatsApp | Fallback e-mail via Resend | 0.5d | S3 |
| T-037 | WhatsApp | Log de mensagens na tabela `messages` | 0.25d | S2 |
| T-038 | Dashboard | Endpoint `GET /api/dashboard/summary` | 1d | S2 |
| T-039 | Dashboard | Cards de KPI de inadimplência | 0.5d | S2 |
| T-040 | Dashboard | Gráfico de evolução (Recharts) | 1d | S2 |
| T-041 | Dashboard | Tabela de inadimplentes + botão cobrar | 1d | S2 |
| T-042 | Dashboard | Atualização real-time via SSE | 1d | S3 |
| T-043 | Qualidade | Setup Sentry front + back | 0.5d | S1 |
| T-044 | Qualidade | Rate limiting global 100 req/min | 0.5d | S1 |
| T-045 | Qualidade | HTTPS + HSTS + CSP | 0.25d | S1 |
| T-046 | Qualidade | Criptografia CPF/telefone AES-256 | 1d | S1 |
| T-047 | Qualidade | Pipeline CI GitHub Actions | 0.5d | S1 |
| T-048 | Qualidade | Testes E2E Playwright (fluxos críticos) | 2d | S3 |
