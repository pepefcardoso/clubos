...
describe("seedChecklistItems", () => {
  it("calls createMany with exactly DEFAULT_CHECKLIST_ITEMS.length rows", async () => {
    const tx = makeTx();
  ...
  it("every row carries the supplied eventId", async () => {
    const tx = makeTx();
    await seedChecklistItems(
      tx as unknown as Parameters<typeof listChecklist>[0],
      EVENT_ID,
  ...
  it("each row has a unique id (randomUUID)", async () => {
    const tx = makeTx();
    await seedChecklistItems(
      tx as unknown as Parameters<typeof listChecklist>[0],
      EVENT_ID,
  ...
  it("each row has category and item from DEFAULT_CHECKLIST_ITEMS", async () => {
    const tx = makeTx();
    await seedChecklistItems(
      tx as unknown as Parameters<typeof listChecklist>[0],
      EVENT_ID,
    ...
    const { data } = vi.mocked(tx.gameChecklist.createMany).mock
      .calls[0]![0] as {
      data: Array<{ category: string; item: string }>;
    };
    for (const [idx, def] of DEFAULT_CHECKLIST_ITEMS.entries()) {
      expect(data[idx]!.category).toBe(def.category);
      expect(data[idx]!.item).toBe(def.item);
    }
  });
});

describe("listChecklist", () => {
  it("returns grouped byCategory with correct counts", async () => {
  ...
  it("counts completedItems correctly when mix of completed/pending", async () => {
    const completedItem = { ...BASE_ITEM, id: "item_02", completed: true };
  ...
  it("returns empty byCategory when no items exist", async () => {
    vi.mocked(withTenantSchema).mockImplementationOnce((_p, _c, fn) =>
      fn({
        gameChecklist: { findMany: vi.fn().mockResolvedValue([]) },
      } as unknown as Parameters<typeof fn>[0]),
    );
  ...
  it("groups items with different categories into separate buckets", async () => {
    const items = [
      { ...BASE_ITEM, id: "i1", category: "EQUIPAMENTOS" },
      { ...BASE_ITEM, id: "i2", category: "LOGÍSTICA" },
      { ...BASE_ITEM, id: "i3", category: "EQUIPAMENTOS" },
    ];
    vi.mocked(withTenantSchema).mockImplementationOnce((_p, _c, fn) =>
      fn({
        gameChecklist: { findMany: vi.fn().mockResolvedValue(items) },
      } as unknown as Parameters<typeof fn>[0]),
    );

    const result = await listChecklist(mockPrisma, CLUB_ID, EVENT_ID);

    expect(result.byCategory["EQUIPAMENTOS"]).toHaveLength(2);
    expect(result.byCategory["LOGÍSTICA"]).toHaveLength(1);
  });
...
describe("toggleChecklistItem", () => {
  it("sets completed=true with completedBy and completedAt", async () => {
    ...
    vi.mocked(withTenantSchema).mockImplementationOnce((_p, _c, fn) =>
      fn(makeTx(updatedItem) as unknown as Parameters<typeof fn>[0]),
    );
  ...
  it("clears completedBy and completedAt when completed=false", async () => {
    ...
    vi.mocked(withTenantSchema).mockImplementationOnce((_p, _c, fn) =>
      fn(makeTx(clearedItem) as unknown as Parameters<typeof fn>[0]),
    );
  ...
  it("passes correct data fields to prisma update", async () => {
    const tx = makeTx();
    vi.mocked(withTenantSchema).mockImplementationOnce((_p, _c, fn) =>
    ...
    const call = vi.mocked(tx.gameChecklist.update).mock.calls[0]![0];
    expect(call.data.completedBy).toBe(ACTOR_ID);
    expect(call.data.completed).toBe(true);
    expect(call.where).toEqual({ id: ITEM_ID });
  });

  it("throws NotFoundError when itemId does not belong to eventId", async () => {
    vi.mocked(withTenantSchema).mockImplementationOnce((_p, _c, fn) =>
      fn(makeTx(null) as unknown as Parameters<typeof fn>[0]),
    );
