...
export function TryoutForm({ clubSlug, clubName }: TryoutFormProps) {
  ...
  const handleFormSubmit = async (data: TryoutFormValues) => {
    if (isMinor && !consentToken) {
      ...
      >
        ...
        <div>
          ...
          <p className="text-neutral-500 text-sm max-w-xs mx-auto leading-relaxed">
            O <span className="font-semibold text-neutral-700">{clubName}</span>{" "}
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-3 pt-1">
          {isMinor && consentToken && (
            <div
              role="status"
              className="flex items-center gap-2 text-xs text-primary-700 bg-primary-50 border border-primary-200 rounded-lg px-3 py-2"
            >
              <ShieldCheck
                size={13}
                className="flex-shrink-0"
                aria-hidden="true"
              />
              Consentimento parental registado com sucesso.
            </div>
          )}
          ...
          >
            {isSubmitting ? (
              ...
              "Continuar para o aceite parental"
            ) : (
              "Enviar inscrição"
            )}
          </Button>
      ...
      {showConsentModal && isMinor && (
        <ParentalConsentModal
          clubSlug={clubSlug}
          athleteName={athleteNameValue ?? ""}
          guardianName={guardianNameValue ?? ""}
          guardianPhone={(guardianPhoneValue ?? "").replace(/\D/g, "")}
          guardianRelationship={
            (guardianRelationshipValue as
              | "mae"
              | "pai"
              | "avo"
              | "tio"
