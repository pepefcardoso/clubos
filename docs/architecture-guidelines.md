...
### Convenções do Módulo de Pagamentos

| Regra                                                | Detalhe                                                                      |
| ---------------------------------------------------- | ---------------------------------------------------------------------------- |
| `ChargeService` nunca importa gateways concretos     | Usar sempre `GatewayRegistry.forMethod()` ou `GatewayRegistry.get()`         |
| Novos gateways ficam em `modules/payments/gateways/` | Um arquivo por provider: `<provider>.gateway.ts`                             |
| Bootstrap de gateways centralizado                   | Apenas `gateways/index.ts` instancia e registra — nunca no módulo de negócio |
| Metadados do provider no `gatewayMeta` (JSONB)       | Nunca adicionar colunas específicas de provider no schema                    |
...
4. **Aprovações:** mínimo 1 para merge. Em código financeiro (charges, payments, webhooks, qualquer gateway): mínimo 2 aprovações.
